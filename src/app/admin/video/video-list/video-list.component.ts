...
export class VideoListComponent implements OnInit {
  ...
  getNotificationRecords() {
    ...
    this.http.getUserObservable().pipe(
			mergeMap((user) => {
				if (user) {
					return this.http.setModule('video').list(this.searchParams);
				}
			})
		).subscribe((response) => {
	...
	editSpeaker(video): void {
    this.myRoute.navigate([`admin/video/edit/${video.id}`]);
  }
  ...
  changeActiveStaus(isActiveStatus, id, index) {
    ...
    this.http.setModule('video').update(this.formObj).subscribe((response) => {
			if (response['status'] === 'success') {
  ...
  deleteRecord(id) {
		if (id) {
				...
				this.http.setModule('video').deleteOne({id: id}).subscribe((response) => {
					if (response) {
