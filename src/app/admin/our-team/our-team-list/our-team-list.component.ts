...
export class OurTeamListComponent implements OnInit {
  ...
  getOurTeamRecords() {
    ...
    this.http.getUserObservable().pipe(
			mergeMap((user) => {
				if (user) {
					return this.http.setModule('ourTeam').list(this.searchParams);
				}
			})
		).subscribe((response) => {
	...
	editTeam(team): void {
    this.myRoute.navigate([`admin/our-team/edit/${team.id}`]);
  }
  ...
  changeActiveStaus(isActiveStatus, id, index) {
    ...
    this.http.setModule('ourTeam').update(this.formObj).subscribe((response) => {
			if (response['status'] === 'success') {
  ...
  deleteRecord(id) {
		if (id) {
				...
				this.http.setModule('ourTeam').deleteOne({id: id}).subscribe((response) => {
					if (response) {
						this.toasterService.pop('success', 'Member deleted');
					...
					this.toasterService.pop('error', 'Failed to delete member');
				});
			
		} else {
			this.toasterService.pop('error', 'Member is not available');
		}
	}
