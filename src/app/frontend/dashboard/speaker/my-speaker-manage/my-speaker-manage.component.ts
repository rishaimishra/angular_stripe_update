...
@Component({
  selector: 'app-my-speaker-manage',
  templateUrl: './my-speaker-manage.component.html',
  styleUrls: ['./my-speaker-manage.component.scss']
})
export class MySpeakerManageComponent implements OnInit {
  ...
  public fileInstance: any  = null;
	...
	ngOnInit() {
      ...
      is_active: [1],
			...
			logged_in_user_id:[this.httpService.getUser().id]
		});
	...
	setFormData() {
		this.route.params.pipe(
			mergeMap((params) => {
				if (params) {
					return this.httpService.setModule('speaker').findOne(params.id);
				} else {
					return of(null);
				}
			})
        ...
        this.imageSrc = data.avatar;
        this.thumbnail = data.avatar;
        this.croppedImage = data.avatar;
        this.formObj.value.avatar = data.avatar;
				// console.log(data);
  ...
  const file 		= this.fileInstance;
    this.s3.cropedImageUpload(file).send((err, data) => {
      if (err) {
        ...
        this.formObj.value.avatar = this.thumbnail;
      }
    });
  }
	saveForm() {
	...
	createProduct() {
		this.httpService.setModule('speaker').create(this.formObj.value).subscribe((response) => {
			if (response) {
				this.formLoading = false;
				this.toasterService.pop('success', 'Speaker saved');
				this.router.navigate(['/dashboard', 'my-speaker-list']);
			}
		}, (error) => {
			this.formLoading = false;
			this.formError = null;
	...
	updateProduct() {
		...
		delete params.slug;
		this.httpService.setModule('speaker').update(this.formObj.value).subscribe((response) => {
			if (response) {
				this.formLoading = false;
				this.toasterService.pop('success', 'Speaker updated');
				this.router.navigate(['/dashboard', 'my-speaker-list']);
			}
		}, (error) => {
			this.formLoading = false;
